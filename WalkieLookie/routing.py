...
def center_nodes(street_graph, places_nodes_on_map):
    ...
    try:
        return nx.center(street_graph.subgraph(places_nodes_on_map))[
            0
        ]  # take first center node (in case there are more than 1)
    except:
...
def get_noi(street_graph, places_noi):
    ...
    places_noi = places_noi[
        places_noi.park_nodes_within_graph.apply(lambda x: len(x)) > 0
    ].copy()  # Filter out empty parks
...
def add_start_end_node(start_address, street_graph, nodes_to_visit, end_address=None):
    ...
    orig_node = ox.distance.nearest_nodes(
        street_graph, start_latlng[1], start_latlng[0]
    )  # graph, long, lat
...
def inital_nodes_to_consider(
    user_time, nodes_to_visit_final, street_graph, optimizer="time", avg_speed=5
):
    """
    ...
    for node in nodes_to_visit_final[1:]:
        ...
        shortest_route = nx.shortest_path(
            street_graph, start_node, node, weight=optimizer
        )

        # Calculate route statistics for each node
        length_m, travel_time_min = get_route_stats(
            shortest_route, street_graph, avg_speed
        )
...
def create_walking_route(
    street_graph, start_node, notes_to_visit_small, round_trip, avg_speed=5
):
    """
    ...
    for i in range(iterrations):
        ...
        for node in notes_to_visit_small:
            ...
            # Calculate route statistics for each node
            length_m, travel_time_min = get_route_stats(
                shortest_route, street_graph, avg_speed=5
            )
    ...
    if round_trip == True:
        ...
        shortest_route = nx.shortest_path(
            street_graph, start_node, nodes_to_visit_final[0], weight=optimizer
        )
...
def evaluate_iterrate_route(
    ...
    while not (
        (travel_time_min <= user_time + time_margin)
        & (travel_time_min >= user_time - time_margin)
    ) | (x_init == x):
...
if __name__ == "__main__":
    ...
    filename_places = os.path.join(
        dirname, "data", "bln_xberg_parks_park_nodes_wo_NaN.csv"
    )
    ...
    nodes_to_visit_final = add_start_end_node(
        start_address, street_graph, nodes_to_visit
    )
    (
        notes_to_visit_small,
        notes_to_visit_sorted,
